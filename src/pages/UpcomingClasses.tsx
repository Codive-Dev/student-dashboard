...
export function UpcomingClasses() {
  ...
  const filteredClasses = classes.filter(class_ => {
    const matchesSearch = class_.title.toLowerCase().includes(filters.search.toLowerCase()) ||
      class_.teacher.toLowerCase().includes(filters.search.toLowerCase());
    const matchesSubject = !filters.subject || class_.subject === filters.subject;
    const matchesInstructor = !filters.instructor || class_.teacher === filters.instructor;
    ...
    switch (filters.timeRange) {
      case 'today':
        matchesTimeRange = format(classDate, 'yyyy-MM-dd') === format(now, 'yyyy-MM-dd');
        break;
      case 'week':
    ...
    return matchesSearch && matchesSubject && matchesInstructor && matchesTimeRange;
  });
  ...
  return (
    <div className="max-w-7xl mx-auto px-9 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Upcoming Classes</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 mb-8">
        ...
        {/* Active filters */}
        {(filters.search || filters.subject || filters.instructor || filters.timeRange !== 'all') && (
